...
// Disable keyboard shortcuts (Ctrl+S, Ctrl+P, Ctrl+C, Ctrl+U, F12, PrintScreen)
document.addEventListener('keydown', function(e) {
  const isModifier = e.ctrlKey || e.metaKey; // Handles Windows/Linux (Ctrl) and Mac (Cmd)
  if (
    (isModifier && (e.key === 's' || e.key === 'S')) || // Save
    (isModifier && (e.key === 'p' || e.key === 'P')) || // Print
    (isModifier && (e.key === 'c' || e.key === 'C')) || // Copy
...
secureOverlay.style.transition = 'opacity 0.1s ease';
...
// 2. Hide content when window loses focus (e.g. Snipping tool, desktop app switcher)
...
// 4. Hide content during visibility change (crucial for Mobile browsers and iOS Screenshots)
...
// 5. Overwrite clipboard if user tries to copy (fallback)
